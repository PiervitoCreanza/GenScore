...
def parse_user_input():
    ...
    p.add_argument('-rc', '--refined_csv', help="The csv file containing only the ids of the refined set", required=True)
    p.add_argument('-dg', '--general_set_dir', default=".",
                   help='The directory of the general set', required=True)
    args = p.parse_args()
...
def score_compound(pdbid, ligids, args):
    ...
    with alive_bar(len(ligids)) as bar:
        for ligid in ligids:
            ids, scores = scoring(prot=get_prot_path(args, pdbid),
...
def main():
    ...
    pdbids_refinedSet = get_directories_inside_dir(args.dir)
    pdbids_generalSet = get_directories_inside_dir(args.general_set_dir)
    #ligids_casfCoreSet = ligids_casfCoreSet[:1]
    # intersecting_pdbids = intersection(pdbids_casfDecoys, pdbids_customSet)
    intersecting_pdbids = intersection(pdbids_casfDecoys, pdbids_refinedSet + pdbids_generalSet)
